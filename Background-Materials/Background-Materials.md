...
### Pull-up and Pull-down Resistors
- Helps control current and voltage to protect the GPIO pin
- A high-impedance state occurs when the pin is not pulled to a high or low logic level, but is left “floating" instead
- the pull up and down resistors helps us avoid the "floating" value
- With a pull-up resistor, the default value is 1, 0 with a pull-down
![](images/pullup-dowmn.png)
Sources:
...
- Uses digital input (1 and 0)
    - As input, peripheral requires external power
    - As output, typically provides peripheral with some sort of power
        - note: sometimes external power is still required
...
## ADC
The Analogue-to-Digital Converter, (ADC) allows micro-processor controlled circuits, Arduinos, Raspberry Pi, and other such digital logic circuits to communicate with the real world. In the real world, analogue signals have continuously changing values which come from various sources.

Sensors which can measure sound, light, temperature or movement, etc. Electronic circuits can interact with their environment by measuring the analogue signals from such transducers.

While analogue signals can be continuous and provide an infinite number different voltage values, digital electronic circuits on the other hand work with binary signal which have only two discrete states, a logic “1” (HIGH) or a logic “0” (LOW).
